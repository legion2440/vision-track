...
For NVIDIA CUDA, install a current NVIDIA driver and use the command generated by the official [PyTorch installation selector](https://pytorch.org/get-started/locally/) for Windows, pip, Python, and the CUDA platform supported by the driver. Keep the installed PyTorch version aligned with `torch==2.12.1`.
...
For CUDA, use the official PyTorch selector after installing a compatible NVIDIA driver. CPU remains a mandatory and tested fallback.
...
Tests cover device priority/fallback, annotations, detection filtering/format, frame dropping, independent tracking/counting state, composite IDs, line crossing, duplicate protection, metrics/schema, lifecycle, credential masking, local video integration, two streams, broken plus working sources, EOF, stop/restart/remove/replace, Streamlit state reuse, ONNX inference, audit imports, and Streamlit startup.

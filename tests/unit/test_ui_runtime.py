...
def test_scheduler_publishes_rendered_version_atomically(monkeypatch) -> None:
    ...
    assert context.latest_rendered_version == (8, 17)
...
def test_stale_packet_does_not_publish_version(monkeypatch) -> None:
    ...
    context.latest_rendered_version = (9, 1)
    ...
    assert context.latest_rendered_version == (9, 1)
...
def test_cache_survives_reset_until_first_new_frame(monkeypatch) -> None:
    ...
    context.latest_rendered_frame = np.full((4, 4, 3), 33, dtype=np.uint8)
    context.latest_rendered_version = (4, 0)
    ...
    assert cache[context.stream_id].frame_version == (4, 0)
